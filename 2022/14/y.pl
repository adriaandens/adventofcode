...
draw_paths((500-$lowest_rock-3) . ',' . ($lowest_rock+2), (500+$lowest_rock+3) . ',' . ($lowest_rock+2));
draw_map();
...
say "Solution: " . ($sand_units);

# Returns 1 if we're gonna write to the start position, otherwise 0
sub pour_sand {
	my ($c, $r) = @_;
