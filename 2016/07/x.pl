...
while(<DATA>) {
	...
	my $abba_found = 0;
	my $hypernet_abba_found = 0;
	for(my $i = 0; $i < scalar(@chars) - 3; $i++) {
		if($chars[$i] eq '[') {
			$inside_brackets = 1;
		} elsif($chars[$i] eq ']') {
			$inside_brackets = 0;
		} else {
			if($chars[$i] ne $chars[$i+1] && $chars[$i] eq $chars[$i+3] && $chars[$i+1] eq $chars[$i+2]) {
				$abba_found = 1 if !$inside_brackets;
				$hypernet_abba_found = 1 if $inside_brackets;
			}
		}
	}
	if($abba_found && !$hypernet_abba_found) {
		$support_tls++;
	}
}
print "Sol: $support_tls\n";
