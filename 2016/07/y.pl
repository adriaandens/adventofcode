...
while(<DATA>) {
	...
	my %aba = ();
	my %bab = ();
	for(my $i = 0; $i < scalar(@chars) - 2; $i++) {
		if($chars[$i] eq '[') {
			$inside_brackets = 1;
		} elsif($chars[$i] eq ']') {
			$inside_brackets = 0;
		} else {
			if(!$inside_brackets && $chars[$i] eq $chars[$i+2] && $chars[$i] ne $chars[$i+1] && $chars[$i] ne '[' && $chars[$i+1] ne '[' && $chars[$i] ne ']' && $chars[$i+1] ne ']') {
				$aba{$chars[$i] . $chars[$i+1] . $chars[$i+2]} = 1;
			} elsif($inside_brackets && $chars[$i] eq $chars[$i+2] && $chars[$i] ne $chars[$i+1] && $chars[$i] ne '[' && $chars[$i+1] ne '[' && $chars[$i] ne ']' && $chars[$i+1] ne ']') {
				# Trick: we immediately convert the BAB to an ABA
				$bab{$chars[$i+1] . $chars[$i] . $chars[$i+1]} = 1;
			}
		}
	}
	my @babs = keys(%bab);
	foreach my $a (keys(%aba)) {
		if($bab{$a}) {
			print $str;
			print "\tBABs: @babs $/";
			print "\tABA: $a MATCHED\n";
