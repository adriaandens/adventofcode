...
while(<DATA>) {
	chomp;
	my @dashsplit = split /-/;
	my ($sectorid, $checksum) = ($dashsplit[$#dashsplit] =~ /^(\d+)\[(\w+)/);
	for(my $i = 0; $i < scalar(@dashsplit) - 1; $i++) {
		my @chars = split //, $dashsplit[$i];
		foreach(@chars) {
			$amount{$_}++;
		}
	}
	my @sorted = sort sortjedoen keys(%amount);
	my @checkie = split //, $checksum;
	if($checkie[0] eq $sorted[0] && $checkie[1] eq $sorted[1] && $checkie[2] eq $sorted[2] && $checkie[3] eq $sorted[3] && $checkie[4] eq $sorted[4]) {
		pop @dashsplit; # after this pop we have our string
		my @decrypted = ();
		foreach(split //, join(' ', @dashsplit)) {
			if($_ eq ' ') {
				push @decrypted, ' ';
			} else {
				push @decrypted, chr(((ord($_) - 97 + $sectorid) % 26) + 97);
			}
		}
		print join('', @decrypted) . " - $sectorid" . $/;
	}
	%amount = (); # reset
}
...
sub sortjedoen {
	if($amount{$a} > $amount{$b}) {
		return -1;
	} elsif($amount{$a} < $amount{$b}) {
		return 1;
	} elsif($amount{$a} == $amount{$b} && ord($a) > ord($b)) {
		return 1;
	} elsif($amount{$a} == $amount{$b} && ord($a) < ord($b)) {
		return -1;
	} else {
