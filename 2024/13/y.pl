...
while(<DATA>) {
	if(m/Button A/) {
		...
		my ($x, $y) = (m/X=(\d+), Y=(\d+)/);
		my %machine = ();
		$machine{x} = $x + 10000000000000; $machine{y} = $y + 10000000000000;
		$machine{ax} = $rax;
		$machine{ay} = $ray;
		$machine{bx} = $rbx;
		$machine{by} = $rby;
...
foreach my $m (@machines) {
	say $m->{x}; say $m->{ax};
	# a = ($x (8400) - $bx (22)b) / $ax (94)

	my $b = ($m->{ax}*$m->{y} - $m->{x}*$m->{ay}) / ($m->{ax}*$m->{by} - $m->{ay}*$m->{bx});
	my $a = ($m->{x} - $m->{bx}*$b) / $m->{ax};
	say "\tA = $a, B = $b";
	next if $a < 0 || $b < 0;
	next if !isint($a) || !isint($b);
	$tokens += $a*3 + $b*1;
}
say "Sol: $tokens";
