...
sub go {
	...
	my ($p, $m, $mem) = @_;
	return 1 if $p eq $m;
	# 1. remove already matched prefix from pattern
	my $unmatched = substr $p, length($m);
	if($mem->{$unmatched . '#' . length($m)}) {
		return $mem->{$unmatched . '#' . length($m)};
	}
	# go through the sorted towels list and for each candidate that can match the next character(s):
	my $total = 0;
	foreach(@sorted_towels) {
		my $a = ord(substr($_, 0, 1)); my $b = ord(substr($unmatched, 0, 1));
		if($a < $b) {
			next
		} elsif($a > $b) {
			last
		} else {
			if(substr($unmatched, 0, length) eq $_) {
				my $is_a_match = go($p, $m . $_, $mem);
				if($is_a_match) {
					$total += $is_a_match;
				}
			}
		}
		# call go() again with the new matched prefix
	}
	$mem->{$unmatched . '#' . length($m)} = $total;
	return $total;
}
