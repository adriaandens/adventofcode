...
# If the increase is tenfold then if there is an empty row, we need to add an extra 9 to make 10 empty rows.
sub expand_universe {
	...
	foreach my $co (@coordinates) {
		...
		if(defined($prev_co) && $prev_co->{row} - (($increase_row*$increase)-$increase_row) < $co->{row} - 1) {
			say print_co($prev_co);
			$increase_row++;
		}
		if($increase_row) {
			# If we have to increase 1 row, add 9 rows in between
			# If we have to increase 2 rows, those become 9+9 new rows, so 18, not 19 (don't do -1)
			$co->{row} += ($increase_row * $increase) - $increase_row;
		}
		$prev_co = $co;
	}
		...
		while($i < $colcount) {
			if(!$cols_with_galaxies{$i}) {
				...
				foreach my $co (@coordinates) {
					...
					if($co->{col} - (($increase_col*$increase)-$increase_col) > $i) {
						$co->{col} += $increase-1;
					}
				}
				$increase_col++;
